...
export const useBackendFilters = (): UseBackendFiltersReturn => {
  const backendFilters = useAppSelector(selectBackendFilters) as BackendFilters | null;
  ...
  const wheelsFilters = useMemo(() => {
    ...
    return {
      ...
      wheel_drives: (wheels.drives || wheels.wheel_drives) 
        ? extractNames(wheels.drives || wheels.wheel_drives) 
        : undefined,
      wheels_fixing_points: (wheels.fixing_points || wheels.wheels_fixing_points)
        ? extractNames(wheels.fixing_points || wheels.wheels_fixing_points)
        : undefined,
      wheels_spacing: (wheels.spacing || wheels.wheels_spacing)
        ? extractNames(wheels.spacing || wheels.wheels_spacing)
        : undefined,
      wheels_central_diameter: (wheels.central_diameter || wheels.wheels_central_diameter)
        ? extractNames(wheels.central_diameter || wheels.wheels_central_diameter)
        : undefined,
      wheels_width: (wheels.width || wheels.wheels_width)
        ? extractNames(wheels.width || wheels.wheels_width)
        : undefined,
      wheels_height: (wheels.height || wheels.wheels_height)
        ? extractNames(wheels.height || wheels.wheels_height)
        : undefined,
      wheels_tread_depth: (wheels.tread_depth || wheels.wheels_tread_depth)
        ? extractNames(wheels.tread_depth || wheels.wheels_tread_depth)
        : undefined,
    };
  }, [backendFilters]);
