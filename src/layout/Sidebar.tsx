import { LayoutDashboard, Package, Car, ShoppingCart, RotateCcw, BarChart3, Menu } from 'lucide-react';
import { useAppDispatch, useAppSelector } from '@/store/hooks';
import { selectCurrentView, selectMetrics, selectSidebarCollapsed } from '@/store/selectors';
  ...
  return (
    <div className={cn(
      ...
      <div className={cn(
        ...
        {!isCollapsed && (
          <h1 className="text-xl font-bold ml-2 truncate min-w-0 flex-1">RRR Car Parts</h1>
        )}
      </div>
      <nav className="flex-1 space-y-1 px-3 py-4">
        {navigation.map((item) => {
          ...
          const showBadge = item.view === 'parts' && metrics.partsOlderThan6Months > 0;
          return (
            <button
              key={item.name}
